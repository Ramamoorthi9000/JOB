...
def jobhire(request):
    if request.method == "POST":
        # Get the social account of the logged-in user
        social_account = SocialAccount.objects.filter(user=request.user).first()

        # Create a new job linked to this social account
...
def views_job(request):
    jobs = job_list.objects.filter(social_account__user=request.user)
